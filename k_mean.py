...
vectors = tf.constant(vectors_set) # 모든 데이타를 테서로 옮긴다.
k = 4 #입력데이터에서 무작위로 k개의 데이터를 선택함. 여기서는 4개
centroids = tf.Variable(tf.slice(tf.random_shuffle(vectors), [0,0], [k, -1]))
# k개의 데이터 포인터는 2D 텐서로 저장됨
print('--- 텐서의 구조 보기---')
print(vectors.get_shape())
# D0 차원의 크기가 앞의 수이고, D1 차원은 vectors와 동일한 2인 행렬
print(centroids.get_shape())
# 센트로이드는 D0 차원의 크기가 4개이고, D1 차원은 vectors 와 동일한 2인 행렬

expended_vectors = tf.expand_dims(vectors, 0) # 2 텐서에 차원을 추가
expended_centroids = tf.expand_dims(centroids, 1) # 2테서를 2차원에서 3차원으로 만들어 뺄셈이 가능토록함.


#크기를 맞추는 작업

# 각 점에대한 유클리드 제곱거리 알고리즘의 무한 반복
...
# 휴리스틱 이론
# 불충분한 시간이나 데이터로 인해 합리적인 판단이 불가능하거나 굳이 필요하지 않은 상태에서 빠르게 사용하는 어림짐작

# 아래 로직은 초기값 결정 -> 가까운 클러스터별 데이터 할당 -> 각 클러스트에 새로운 센트로이드 계산
# 진정한 최저값인지 보장은 없으며 초기 세트로이드 결정에 따하 다른 결과가 나올수 있다.
# 하지만 매우 빠른 결정을 보인다.

#gather  = tf.gather(vectors, )
#temp = tf.where(tf.equal(assignments, c))
#gather = tf.gather(vectors, tf.reshape())
#means = tf.concat([tf.reduce_mean(gather)])
